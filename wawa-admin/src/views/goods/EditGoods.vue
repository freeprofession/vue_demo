<template>
  <div>
    <a-form layout="horizontal" @submit="handleSubmit" :autoFormCreate="(form) => this.form = form" class="form">
        ...
        <a-row class="form-row" :gutter="16">
          ...
          <a-col :md="12" :sm="24">
            <a-form-item
              ...
              :fieldDecoratorOptions="{
                rules: [{ required: false, message: '请输入条形码', whitespace: true}],
                initialValue: goods.barcode
              }"
            >
          ...
          <a-col :md="12" :sm="24">
            <a-form-item
              ...
              :fieldDecoratorOptions="{
                rules: [{ required: false, message: '请输入商品别名'}],
                initialValue: goods.alias
              }"
            >
          ...
          <a-col :md="12" :sm="24">
            <a-form-item
              ...
              initialValue: goods.supplier_id
              }"
            >
          ...
          <a-col :md="12" :sm="24">
            <a-form-item
              ...
              :fieldDecoratorOptions="{
                rules: [{ required: true, message: '请输入兑换规则'}],
          ...
          <a-col :md="12" :sm="24">
            <a-form-item
              ...
              :fieldDecoratorOptions="{
                ...
                initialValue: goods.major_id
              }"
            >
              <a-select placeholder="请选择主分类">
          ...
          <a-col :md="12" :sm="24" v-if="is_online > 0">
            <a-form-item
              ...
              :fieldDecoratorOptions="{
                rules: [{ required: true, message: '请选择邮费规则'}],
                initialValue: goods.template_id
              }"
            >
          ...
          <a-col :md="12" :sm="24">
            <a-form-item
              ...
              :fieldDecoratorOptions="{
                rules: [{ required: true, message: '请选择副分类'}],
                initialValue: goods.minor_id
              }"
            >
          ...
          <!-- <a-col :md="12" :sm="24">
            <a-form-item
              ...
              :fieldDecoratorOptions="{
                rules: [{ required: false, message: '请输入拼团起始时间'}],
                initialValue: goods.online_time
              }"
            >
          ...
          <a-col :md="12" :sm="24">
            <a-form-item
              ...
              :fieldDecoratorOptions="{
                rules: [{ required: false, message: '请输入拼团期限'}],
                initialValue: goods.period
              }"
            >
          ...
          <a-col :md="12" :sm="24">
            <a-form-item
              ...
              :fieldDecoratorOptions="{
                rules: [{ required: false, message: '请输入目标单数'}],
                initialValue: goods.required_number
              }"
            >
      ...
      <a-card class="card" title="价格信息" :bordered="false">
        <a-table :columns="columns" :dataSource="skus" :pagination="false" bordered>
          ...
          <template slot="min_quantity" slot-scope="text, record">
            <a-input v-model.number="record.min_quantity" style="width:50px;"/>
          </template>
          <template slot="purchase_price" slot-scope="text, record">
            <a-input v-model="record.purchase_price" style="width: 100px;"/>
          </template>
          ...
          <template slot="action" slot-scope="text, record">
            <div class='editable-row-operations'>
              <a-switch v-model="record.deleted" @change="handleDeleteChange()" />
            </div>
          </template>
        </a-table>
...
export default {
  ...
  methods: {
    getGoods() {
      const vm = this
      getGoods(vm.$route.params.id).then(res => {
        if (res.code === 0) {
          vm.goods = res.data
          vm.pic_url = vm.goods.pic_url
          vm.fileList = vm.goods.images.map(image => {
            return {
              uid: image.id,
              url: image.url,
              status: 'done'
          ...
          vm.detail = vm.goods.detail
          vm.skus  = vm.goods.skus
          vm.is_online = vm.goods.is_online
        }
      })
    },
    ...
    handleSubmit (e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          values.images = this.images.concat(vm.fileList.map(item => {
            let pic_id = 0
            let url = ''
            if(isNumber(item.uid)) {
              ...
              url = vm.bucketHost + item.response.key
            }
            return {
              url,
              pic_id
          ...
          values.skus = vm.skus
          values.is_online = vm.is_online
          updateGoods(vm.goods.id, values).then(res => {
            if (res.code === 0) {
    ...
    beforeCoverUpload: function () {
      ...
      return getQiniuToken().then(res => {
        vm.bucketHost = res.data.bucketHost
        vm.imgForm = {
          // key: res.key,
