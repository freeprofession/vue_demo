  ...
  <a-card :bordered="false">
    <div class="table-page-search-wrapper">
      <a-form layout="inline" @submit="handleSearch" :autoFormCreate="(form)=>{this.searchForm = form}">
        <a-row :gutter="48">
          ...
          <a-col :md="6" :sm="24">
              <a-form-item label="差异筛选" fieldDecoratorId="difference">
                <a-select placeholder="请选择">
    ...
    >
      ...
      <template slot="difference" slot-scope="text, record">
        <p>{{ record.check_stock ? record.check_stock-record.unit_stock : text }}</p>
      </template>
      <template slot="action" slot-scope="text, record">
        <div class='editable-row-operations'>
          <a :disabled="record.check" @click="handleSubmit(record)">提交</a>
        </div>
      </template>
...
import { getReviewInventory, reviewInventory } from '@/api/stock'
...
export default {
  ...
  data() {
    return {
      ...
      loadData: params => {
        return getReviewInventory(Object.assign(params, this.queryParam))
          .then(res => {
            return res.data
          })
      },
    ...
    handleSearch(e) {
      ...
      this.queryParam = {
        filter: {
          ...
          difference:this.searchForm.getFieldValue('difference')
        }
      }
