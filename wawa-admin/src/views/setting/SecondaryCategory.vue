...
import { getSecondaryCategoryList, addSecondaryCategory, updateSecondaryCategory, deleteSecondaryCategory } from '@/api/category'
...
export default {
  ...
  data() {
    return {
      ...
      loadData: params => {
        return getSecondaryCategoryList(Object.assign(params, this.queryParam))
          .then(res => {
            return res.data
          })
      },
  ...
  methods: {
    ...
    handleOk () {
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          if (this.mdl.id) {
            updateSecondaryCategory(this.mdl.id, this.mdl).then(res => {
              if (res.code === 0) {
            ...
            addSecondaryCategory(this.mdl).then(res => {
              if (res.code === 0) {
                this.$refs.table.refresh()
    ...
    del (row) {
      ...
      this.$confirm({
        ...
        cancelText: '取消',
        onOk() {
          deleteSecondaryCategory(row.id).then(res => {
            if (res.code === 0) {
