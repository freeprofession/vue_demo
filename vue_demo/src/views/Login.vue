<template>
  <div id="app">
    <div class="dowebok">
      <div class="logo"></div>
      <div class="form-item">
        <input v-model="name" id="username" type="text" autocomplete="off" placeholder="账号">
      </div>
      <div class="form-item">
        <input v-model="password" id="password" type="password" autocomplete="off" placeholder="密码">
      </div>
      <div class="form-item"><button id="submit" @click="sub">登 录</button></div>
      <div class="reg-bar">
  ...
  methods: {
    sub () {
      if (this.name === 'admin' && this.password === '123') {
        this.$message.success('登录成功', 2, () => { this.$router.push ('/') })
      } else {
        this.$message.warning('用户名或登录密码错误')
      }
    }
  }
}
...
<style>
  #app { height: 100%; background: #fff url(../assets/images/backgroud.png) 50% 50% no-repeat; background-size: cover;}
